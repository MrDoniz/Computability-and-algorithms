  ...
  switch (argc) {
    ...
    case 2: {
      ...
      if (command == "--help")
        ...
        std::cout << "This program is a pattern finder, which finds simple patterns within a text\n"
                  << "file and outputs its findings to another file. It requires three parameters\n"
                  << "indicating the pattern and the names for the input and output files.\n"
                  << "Usage: ./pattern_search pattern input.txt output.txt\n";
      return false;
      break;
    }
    default: {
